...
NODE_EXAMS = {
    ...
    "array": {
        "questions": [
            {"id": "arr_1", "question": "数组在内存中的存储方式是什么？", "options": ["连续存储", "链式存储", "树形存储", "散列存储"], "correct": 0, "explanation": "数组元素在内存中是连续存储的，可以通过首地址+偏移量快速访问。"},
            {"id": "arr_2", "question": "数组下标从几开始？", "options": ["0", "1", "视语言而定", "2"], "correct": 2, "explanation": "大多数编程语言数组下标从0开始，但也有语言从1开始（如Fortran, MATLAB）。"},
            {"id": "arr_3", "question": "已知 int a[5]，则 sizeof(a) 的值是？", "options": ["4", "5", "20", "40"], "correct": 2, "explanation": "int 占4字节，5个int共20字节。"},
        ]
    },
    # ★ 别名：linked_list → slist（种子数据 ID 为 slist）
    "linked_list": {"questions": [
        {"id": "ll_1", "question": "单链表中插入一个节点的时间复杂度是？", "options": ["O(1)", "O(n)", "O(log n)", "O(n^2)"], "correct": 0, "explanation": "已知插入位置时，单链表插入为O(1)；但查找插入位置需要O(n)。"},
        {"id": "ll_2", "question": "链表相比数组的主要优势是？", "options": ["随机访问快", "内存连续", "插入删除效率高", "占用内存少"], "correct": 2, "explanation": "链表插入和删除不需要移动元素，只需修改指针。"},
        {"id": "ll_3", "question": "双向链表相比单向链表多了一个什么指针？", "options": ["next", "prev", "head", "tail"], "correct": 1, "explanation": "双向链表比单向链表多了一个指向前驱节点的prev指针。"},
    ]},
    "slist": {"questions": [
        {"id": "sl_1", "question": "单链表中插入一个节点的时间复杂度是？", "options": ["O(1)", "O(n)", "O(log n)", "O(n^2)"], "correct": 0, "explanation": "已知插入位置时，单链表插入为O(1)；但查找插入位置需要O(n)。"},
        {"id": "sl_2", "question": "链表相比数组的主要优势是？", "options": ["随机访问快", "内存连续", "插入删除效率高", "占用内存少"], "correct": 2, "explanation": "链表插入和删除不需要移动元素，只需修改指针。"},
        {"id": "sl_3", "question": "头插法创建链表时，新节点插入到链表的什么位置？", "options": ["头部", "尾部", "中间", "随机位置"], "correct": 0, "explanation": "头插法每次将新节点插入到链表头部，最终链表顺序与插入顺序相反。"},
    ]},
    "dlist": {"questions": [
        {"id": "dl_1", "question": "双向链表每个节点包含几个指针域？", "options": ["1个", "2个", "3个", "4个"], "correct": 1, "explanation": "双向链表每个节点包含前驱指针 prev 和后继指针 next，共2个指针域。"},
        {"id": "dl_2", "question": "在双向链表中删除一个已知节点的时间复杂度是？", "options": ["O(1)", "O(n)", "O(log n)", "O(n²)"], "correct": 0, "explanation": "已知节点时，通过prev和next指针可以直接修改前后节点的链接，无需遍历。"},
        {"id": "dl_3", "question": "双向链表相对于单向链表的主要优势是？", "options": ["节省内存", "支持双向遍历", "插入更快", "查找更快"], "correct": 1, "explanation": "双向链表可以向前和向后两个方向遍历，操作更灵活。"},
    ]},
    "clist": {"questions": [
        {"id": "cl_1", "question": "循环链表的尾节点指针指向哪里？", "options": ["NULL", "头节点", "自身", "随机地址"], "correct": 1, "explanation": "循环链表的尾节点的next指针指向头节点，形成环状结构。"},
        {"id": "cl_2", "question": "循环链表适合解决什么问题？", "options": ["约瑟夫问题", "排序问题", "查找问题", "哈希冲突"], "correct": 0, "explanation": "循环链表适合约瑟夫环等需要循环遍历的场景。"},
        {"id": "cl_3", "question": "循环链表的遍历终止条件是什么？", "options": ["p == NULL", "p.next == head", "p.next == NULL", "count == n"], "correct": 1, "explanation": "循环链表从头节点开始遍历，当回到头节点时表示遍历完成。"},
    ]},
    # ═══ 受限线性结构（栈与队列）═══
    "stack": {
        "questions": [
            {"id": "stk_1", "question": "栈的特点是？", "options": ["先进先出", "先进后出", "随机访问", "双端操作"], "correct": 1, "explanation": "栈是LIFO（后进先出）结构。"},
            {"id": "stk_2", "question": "用栈实现括号匹配，遇到左括号应执行什么操作？", "options": ["入栈", "出栈", "查看栈顶", "清空栈"], "correct": 0, "explanation": "遇到左括号入栈，遇到右括号出栈并匹配。"},
            {"id": "stk_3", "question": "递归函数的调用过程本质上使用了什么数据结构？", "options": ["队列", "栈", "数组", "树"], "correct": 1, "explanation": "函数调用通过调用栈（Call Stack）实现，递归本质是栈的运用。"},
        ]
    },
    "queue": {
        "questions": [
            {"id": "qu_1", "question": "队列的特点是？", "options": ["先进先出", "先进后出", "随机访问", "优先级排序"], "correct": 0, "explanation": "队列是FIFO（先进先出）结构。"},
            {"id": "qu_2", "question": "循环队列解决的主要问题是什么？", "options": ["空间不足", "假溢出", "入队慢", "查找慢"], "correct": 1, "explanation": "循环队列通过取模运算复用数组空间，解决顺序队列的'假溢出'问题。"},
            {"id": "qu_3", "question": "BFS（广度优先搜索）使用什么数据结构辅助？", "options": ["栈", "队列", "数组", "堆"], "correct": 1, "explanation": "BFS利用队列的FIFO特性实现逐层遍历。"},
        ]
    },
    # ═══ 树形结构 ═══
    # ★ 别名：binary_tree → btree
    "binary_tree": {"questions": [
        {"id": "bt_1", "question": "二叉树的深度优先遍历不包括以下哪种？", "options": ["前序遍历", "中序遍历", "后序遍历", "层序遍历"], "correct": 3, "explanation": "深度优先遍历包括前序、中序、后序；层序遍历属于广度优先。"},
        {"id": "bt_2", "question": "完全二叉树中，度为1的节点数最多为？", "options": ["0个", "1个", "2个", "不确定"], "correct": 1, "explanation": "完全二叉树中最多只有一个度为1的节点。"},
        {"id": "bt_3", "question": "一棵有n个节点的完全二叉树，其高度为？", "options": ["n", "log₂n", "⌊log₂n⌋+1", "n/2"], "correct": 2, "explanation": "完全二叉树高度为⌊log₂n⌋+1，保证了O(log n)的查找效率。"},
    ]},
    "btree": {"questions": [
        {"id": "btr_1", "question": "二叉树的深度优先遍历不包括以下哪种？", "options": ["前序遍历", "中序遍历", "后序遍历", "层序遍历"], "correct": 3, "explanation": "深度优先遍历包括前序、中序、后序；层序遍历属于广度优先。"},
        {"id": "btr_2", "question": "二叉树中，叶子节点的度为？", "options": ["0", "1", "2", "视情况而定"], "correct": 0, "explanation": "叶子节点没有子节点，所以度为0。"},
        {"id": "btr_3", "question": "前序遍历中，根节点的访问顺序是？", "options": ["最后访问", "最先访问", "中间访问", "不确定"], "correct": 1, "explanation": "前序遍历的访问顺序是：根节点→左子树→右子树，根最先访问。"},
    ]},
    "bst": {
        "questions": [
            {"id": "bst_1", "question": "二叉搜索树中序遍历的结果是？", "options": ["降序序列", "升序序列", "随机序列", "层序序列"], "correct": 1, "explanation": "二叉搜索树（左<根<右）中序遍历得到升序序列。"},
            {"id": "bst_2", "question": "BST删除有两个子节点的节点时，通常用什么节点替换？", "options": ["左子树最大节点", "右子树最小节点", "父节点", "根节点"], "correct": 1, "explanation": "通常用右子树的最小节点（中序后继）替换，也可用左子树最大节点。"},
            {"id": "bst_3", "question": "在最坏情况下，BST的查找时间复杂度退化为？", "options": ["O(1)", "O(log n)", "O(n)", "O(n²)"], "correct": 2, "explanation": "BST退化为链表时（插入有序序列），查找退化为O(n)。"},
        ]
    },
    "avl": {"questions": [
        {"id": "avl_1", "question": "AVL树的平衡因子范围是？", "options": ["[-1, 1]", "[0, 2]", "[-2, 2]", "[0, 1]"], "correct": 0, "explanation": "AVL树的平衡因子（左右子树高度差）必须在-1到1之间。"},
        {"id": "avl_2", "question": "AVL树中，LR旋转适用于什么场景？", "options": ["左左情况", "左右情况", "右左情况", "右右情况"], "correct": 1, "explanation": "LR旋转（先左旋后右旋）适用于插入节点在左子树的右子树导致失衡的情况。"},
        {"id": "avl_3", "question": "AVL树查找的时间复杂度是？", "options": ["O(1)", "O(log n)", "O(n)", "O(n log n)"], "correct": 1, "explanation": "AVL树严格平衡，树高为O(log n)，查找也是O(log n)。"},
    ]},
    "rbtree": {"questions": [
        {"id": "rb_1", "question": "红黑树中以下哪个说法正确？", "options": ["根节点可以是红色", "红色节点的子节点必须是黑色", "所有叶子节点必须是红色", "相邻节点不能都是黑色"], "correct": 1, "explanation": "红黑树中红色节点的子节点必须是黑色（不能有连续红色节点）。"},
        {"id": "rb_2", "question": "红黑树相比AVL树的主要优势是？", "options": ["查找更快", "完全平衡", "插入删除旋转次数更少", "内存占用更小"], "correct": 2, "explanation": "红黑树放宽平衡条件，插入删除时旋转操作更少，适合频繁写入场景。"},
        {"id": "rb_3", "question": "红黑树的5条性质中，保证树高不超过O(log n)的关键是？", "options": ["根节点为黑", "红节点子节点为黑", "黑高度相同", "叶子节点为黑"], "correct": 2, "explanation": "任何路径的黑节点数相同（黑高度相同），限制了树高不超过2log(n+1)。"},
    ]},
    # ★ 保留合并别名
    ...
    "heap": {
        "questions": [
            {"id": "hp_1", "question": "大顶堆的堆顶元素是？", "options": ["最大值", "最小值", "中间值", "随机值"], "correct": 0, "explanation": "大顶堆的堆顶（根节点）是最大值。"},
            {"id": "hp_2", "question": "堆排序的时间复杂度是？", "options": ["O(n)", "O(n log n)", "O(n²)", "O(log n)"], "correct": 1, "explanation": "堆排序建堆O(n)，每次调整O(log n)，总体O(n log n)。"},
            {"id": "hp_3", "question": "在堆中插入一个元素后，需要执行什么操作？", "options": ["下沉（sink）", "上浮（swim）", "旋转", "交换左右子树"], "correct": 1, "explanation": "插入元素放在末尾后，通过上浮操作（与父节点比较交换）恢复堆序。"},
        ]
    },
    "bplus": {"questions": [
    ...
    "graph": {
        "questions": [
            {"id": "gr_1", "question": "图的深度优先搜索（DFS）使用什么数据结构辅助？", "options": ["队列", "栈", "数组", "链表"], "correct": 1, "explanation": "DFS使用栈（递归本质也是栈）实现，BFS使用队列。"},
            {"id": "gr_2", "question": "一个有n个顶点的连通图至少有多少条边？", "options": ["n-1", "n", "n+1", "n²"], "correct": 0, "explanation": "连通图至少需要n-1条边（树形结构）。"},
            {"id": "gr_3", "question": "邻接表和邻接矩阵的空间复杂度分别是？", "options": ["O(V+E)和O(V²)", "O(V²)和O(V+E)", "O(V)和O(E)", "O(E)和O(V)"], "correct": 0, "explanation": "邻接表O(V+E)，适合稀疏图；邻接矩阵O(V²)，适合稠密图。"},
        ]
    },
...
EXAM_KB_PROMPT = """你是一位教育专家。请根据知识库参考资料，为知识点"{node_title}"生成{question_count}道选择题。
...
2. 每题4个选项（A/B/C/D），只有1个正确答案
3. 每题附带简要解释（为什么对、为什么错）
4. 难度适中，优先考察核心概念和关键知识点
5. 优先用知识库中的概念出题，不要编造
...
def _generate_kb_questions(node_id: str, node_title: str, subject: str = "数据结构") -> Optional[List[dict]]:
    ...
    try:
        ...
        print(f"[Exam] ✅ 基于知识库为 {node_id} 生成了 {len(questions)} 道题")
        return questions
...
def _get_cached_or_generate_questions(node_id: str, node_title: str, subject: str = "数据结构") -> List[dict]:
    ...
    # 回退到硬编码题库
    return NODE_EXAMS.get(node_id, DEFAULT_EXAM)["questions"]
...
async def get_exam_questions(
    ...
    # ★ 优先基于知识库生成（缓存），回退到硬编码题库
    questions_full = _get_cached_or_generate_questions(node_id, node.title)
